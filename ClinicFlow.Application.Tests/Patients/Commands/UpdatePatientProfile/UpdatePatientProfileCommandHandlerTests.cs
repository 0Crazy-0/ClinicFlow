...
public class UpdatePatientProfileCommandHandlerTests
{
    ...
    public UpdatePatientProfileCommandHandlerTests()
    {
        ...
        _sut = new UpdatePatientProfileCommandHandler(_patientRepositoryMock.Object, _unitOfWorkMock.Object);
    }
    ...
    {
        // Arrange
        var command = new UpdatePatientProfileCommand(Guid.NewGuid(), "O+", "Peanuts", "Asthma", "Dad", "555-1234");
        var patient = Patient.CreateSelf(command.PatientId, PersonName.Create("John Doe"), DateTime.UtcNow.AddYears(-30));

        _patientRepositoryMock.Setup(x => x.GetByIdAsync(command.PatientId, It.IsAny<CancellationToken>()))
        ...
        // Assert
        _patientRepositoryMock.Verify(x => x.UpdateAsync(patient, It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task Handle_ShouldThrowException_WhenPatientDoesNotExist()
    {
        ...
        _patientRepositoryMock.Verify(x => x.UpdateAsync(It.IsAny<Patient>(), It.IsAny<CancellationToken>()), Times.Never);
