...
public class CompleteMedicalEncounterCommandValidator : AbstractValidator<CompleteMedicalEncounterCommand>
{
    public CompleteMedicalEncounterCommandValidator()
    {
        ...
        {
            details.RuleFor(d => d.TemplateCode).NotEmpty().WithMessage("Template code is required for each clinical detail.");
            details.RuleFor(d => d.JsonDataPayload).NotEmpty().WithMessage("JSON data payload is required for each clinical detail.");
        });
    }
}
