...
public class CancelAppointmentByDoctorCommandHandlerTests
{
    ...
    {
        _sut = new CancelAppointmentByDoctorCommandHandler(_appointmentRepositoryMock.Object, _doctorRepositoryMock.Object, _specialtyRepositoryMock.Object,
    ...
    {
        // Arrange
        var command = new CancelAppointmentByDoctorCommand(Guid.NewGuid(), Guid.NewGuid(), "Doctor reason");

        var doctorId = Guid.NewGuid();
        var specialtyId = Guid.NewGuid();
        var appointment = CreateAppointment(command.AppointmentId, Guid.NewGuid(), doctorId, Guid.NewGuid());
        ...
        _doctorRepositoryMock.Setup(r => r.GetByUserIdAsync(command.InitiatorUserId, It.IsAny<CancellationToken>())).ReturnsAsync(doctor);
        _specialtyRepositoryMock.Setup(r => r.GetByIdAsync(specialtyId, It.IsAny<CancellationToken>())).ReturnsAsync(CreateSpecialty(specialtyId));
    ...
    {
        var appointment = Appointment.Schedule(patientId, doctorId, typeId, DateTime.UtcNow.AddDays(2).Date,  TimeRange.Create(new TimeSpan(10, 0, 0), new TimeSpan(11, 0, 0)));
