...
public class AppointmentCancelledEventHandlerTests
{
    ...
    public async Task Handle_ShouldApplyPenalty_WhenStatusIsLateCancellation()
    {
        ...
        var appointment = CreateAppointment(appointmentId, Guid.NewGuid(), patientId, Guid.NewGuid(), DateTime.UtcNow.AddHours(2), true);
        ...
        _penaltyRepositoryMock.Setup(x => x.GetByPatientIdAsync(patientId, It.IsAny<CancellationToken>())).ReturnsAsync([]);

        // Act
        await _sut.Handle(notification, CancellationToken.None);

        // Assert
        _penaltyRepositoryMock.Verify(x => x.AddAsync(It.Is<PatientPenalty>(p => p.Type == PenaltyType.Warning), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_ShouldNotApplyPenalty_WhenStatusIsNotLateCancellation()
    {
        ...
        var appointmentId = Guid.NewGuid();
        var patientId = Guid.NewGuid();
        var appointment = CreateAppointment(appointmentId, Guid.NewGuid(), patientId, Guid.NewGuid(), DateTime.UtcNow.AddDays(2), false);
        ...
        // Assert
        _penaltyRepositoryMock.Verify(x => x.GetByPatientIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()), Times.Never);
        _penaltyRepositoryMock.Verify(x => x.AddAsync(It.IsAny<PatientPenalty>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    // Helpers
    private static Appointment CreateAppointment(Guid id, Guid doctorId, Guid patientId, Guid typeId, DateTime scheduledDateTime, bool isLateCancellation)
    {
        ...
        {
            SetPrivateProperty(appointment, nameof(Appointment.Status), AppointmentStatus.LateCancellation);
        }
        else
        {
            SetPrivateProperty(appointment, nameof(Appointment.Status), AppointmentStatus.Cancelled);
        }
