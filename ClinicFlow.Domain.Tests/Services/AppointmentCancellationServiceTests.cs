...
public class AppointmentCancellationServiceTests
{
    ...
    {
        // Arrange
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).Build();
        var initiatorUserId = Guid.NewGuid();
        var args = new StaffCancellationArgs(initiatorUserId, UserRole.Admin, CreateSpecialty(24), "Admin Reason");
    ...
    public void CancelByStaff_ShouldSucceed_WhenReceptionist()
    {
        ...
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).Build();
        var initiatorUserId = Guid.NewGuid();
        var args = new StaffCancellationArgs(initiatorUserId, UserRole.Receptionist, CreateSpecialty(24), "Receptionist Reason");
    ...
    [InlineData("   ")]
    [InlineData(null)]
    public void CancelByStaff_ShouldThrowBusinessRuleValidationException_WhenReasonIsMissing(string? reason)
    {
        // Arrange
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).Build();
        var args = new StaffCancellationArgs(Guid.NewGuid(), UserRole.Admin, CreateSpecialty(24), reason!);
    ...
    public void CancelByStaff_ShouldThrowUnauthorized_WhenRoleIsInvalid()
    {
        // Arrange
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).Build();
        var args = new StaffCancellationArgs(Guid.NewGuid(), UserRole.Patient, CreateSpecialty(24), "Patient Reason");

        // Act
        var act = () => AppointmentCancellationService.CancelByStaff(appointment, args);

        // Assert
        act.Should().Throw<AppointmentCancellationUnauthorizedException>().WithMessage(DomainErrors.Appointment.CannotCancel);
    }
    ...
    public void CancelByPatient_ShouldSucceed_WhenPatientIsSelf(int age, AppointmentCategory category)
    {
        // Arrange
        var initiatorUserId = Guid.NewGuid();
        var patientId = Guid.NewGuid();
        var patient = CreateSelfPatient(patientId, initiatorUserId, age);
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).WithPatientId(patientId).Build();
        var args = new PatientCancellationArgs(patient, patient, category, CreateSpecialty(24), "Patient reason");
    ...
    public void CancelByPatient_ShouldSucceed_WhenPatientIsFamilyMember(PatientRelationship relationship, int age, AppointmentCategory category)
    {
        // Arrange
        var initiatorUserId = Guid.NewGuid();
        var patientId = Guid.NewGuid();
        ...
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).WithPatientId(patientId).Build();
        var args = new PatientCancellationArgs(patient, initiatorPatient, category, CreateSpecialty(24), "Patient reason");
    ...
    public void CancelByPatient_ShouldThrowUnauthorized_WhenPatientIsSelfButCategoryIsProcedure()
    {
        ...
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).WithPatientId(patientId).Build();
        var args = new PatientCancellationArgs(patient, patient, AppointmentCategory.Procedure, CreateSpecialty(24), "Patient reason");
    ...
    public void CancelByPatient_ShouldThrowUnauthorized_WhenPatientIsFamilyMemberAndRulesFail(PatientRelationship relationship, int age, AppointmentCategory category)
    {
        // Arrange
        var initiatorUserId = Guid.NewGuid();
        var patientId = Guid.NewGuid();
    ...
    public void CancelByPatient_ShouldThrowUnauthorized_WhenPatientDoesNotBelongToUser()
    {
        ...
        var initiatorPatient = CreateSelfPatient(Guid.NewGuid(), initiatorUserId, 30);
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).WithPatientId(patientId).Build();
        var args = new PatientCancellationArgs(patient, initiatorPatient, AppointmentCategory.Checkup, CreateSpecialty(24), "Patient reason");
    ...
    public void CancelByPatient_ShouldThrowValidationException_WhenDataMismatch()
    {
        ...
        var patient = CreateSelfPatient(patientId, initiatorUserId, 30);
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).Build();
        var args = new PatientCancellationArgs(patient, patient, AppointmentCategory.Checkup, CreateSpecialty(24), "Patient reason");

        // Act
        var act = () => AppointmentCancellationService.CancelByPatient(appointment, args);

        // Assert
        act.Should().Throw<DomainValidationException>().WithMessage(DomainErrors.Appointment.DataMismatch);
    }
    ...
    public void CancelByPatient_ShouldThrowValidationException_WhenInitiatorHasNoPatientProfile()
    {
        ...
        var patient = CreateSelfPatient(patientId, initiatorUserId, 30);
        var appointment = new AppointmentBuilder().WithScheduledDateTime(DateTime.UtcNow.AddDays(2)).WithPatientId(patientId).Build();
        var args = new PatientCancellationArgs(patient, null, AppointmentCategory.Checkup, CreateSpecialty(24), "Patient reason");
    ...
    private class AppointmentBuilder
    {
        ...
        public AppointmentBuilder WithPatientId(Guid patientId) { _patientId = patientId; return this; }
        public AppointmentBuilder WithDoctorId(Guid doctorId) { _doctorId = doctorId; return this; }
        public AppointmentBuilder WithTypeId(Guid typeId) { _typeId = typeId; return this; }
        public AppointmentBuilder WithScheduledDateTime(DateTime dt) { _scheduledDateTime = dt; return this; }

        public Appointment Build() => Appointment.Schedule(_patientId, _doctorId, _typeId, _scheduledDateTime.Date,
            TimeRange.Create(_scheduledDateTime.TimeOfDay, _scheduledDateTime.TimeOfDay.Add(TimeSpan.FromHours(1))));
    }
    ...
    private static Patient CreateFamilyMemberPatient(Guid id, Guid userId, PatientRelationship relationship, int age)
    {
        var dateOfBirth = DateTime.UtcNow.AddYears(-age);
        var patient = Patient.CreateFamilyMember(userId, PersonName.Create("Test"), relationship, dateOfBirth);
        SetPrivateProperty(patient, nameof(Patient.Id), id);
        return patient;
    }
    ...
    {
        var doctor = Doctor.Create(userId, MedicalLicenseNumber.Create("12345"), Guid.NewGuid(), "555-0000", 101);
