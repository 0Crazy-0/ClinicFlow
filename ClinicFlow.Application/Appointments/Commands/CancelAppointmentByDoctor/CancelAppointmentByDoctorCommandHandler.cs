...
public sealed class CancelAppointmentByDoctorCommandHandler(IAppointmentRepository appointmentRepository, IDoctorRepository doctorRepository,
    IMedicalSpecialtyRepository specialtyRepository, IUnitOfWork unitOfWork) : IRequestHandler<CancelAppointmentByDoctorCommand>
{
    public async Task Handle(CancelAppointmentByDoctorCommand request, CancellationToken cancellationToken)
    {
        ...
        var initiatorDoctor = await doctorRepository.GetByUserIdAsync(request.InitiatorUserId, cancellationToken);

        AppointmentCancellationService.CancelByDoctor(appointment, new DoctorCancellationArgs(initiatorDoctor, specialty, request.Reason));
