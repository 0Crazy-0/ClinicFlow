...
public static class AppointmentNoShowService
{
    ...
    /// <param name="initiator">The user attempting to mark the appointment as a no-show.</param>
    ...
    public static IEnumerable<PatientPenalty> MarkAsNoShow(Appointment appointment, User initiator, IEnumerable<PatientPenalty> existingPenalties)
    {
        ValidateNoShowPermission(appointment.DoctorId, initiator);
    ...
    private static void ValidateNoShowPermission(Guid appointmentDoctorId, User initiator)
    {
        if (initiator.Role is not (UserRole.Admin or UserRole.Receptionist))
        {
            if (initiator.Role is UserRole.Doctor)
            {
                if (initiator.DoctorId != appointmentDoctorId)
