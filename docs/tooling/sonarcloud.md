...
### Issue Exclusions (`sonar.issue.ignore.multicriteria`)
These patterns ignore specific rules on specific paths, used when a rule is fundamentally incompatible with a category of generated code rather than a one-off false positive:
- **`S1186` on Migrations (`**/Migrations/**`):** EF Core generates an empty `Down()` method for migrations that are not designed to be reversible. This was first identified and marked as a false positive on a specific migration file (see [Resolved False Positives](#s1186-empty-method)), and was subsequently generalized into a directory-wide exclusion so future migrations are not flagged individually.
...
> **Note:** This was marked as a false positive *before* the `S1186` rule was excluded for the entire `Migrations` directory (see [Issue Exclusions](#issue-exclusions-sonarissueignoremulticriteria)). It is documented here as the individual case that prompted the broader exclusion. Future migrations will not trigger this rule at all, since the directory-wide exclusion now applies.
