...
public static class AppointmentSchedulingService
{
    ...
    public static Appointment ScheduleAppointment(Patient patient, AppointmentSchedulingArgs args, AppointmentSchedulingContext context)
    {
        patient.EnsureCompleteProfile();
        Patient.EnsureNotBlocked(context.Penalties);

        EnsureDoctorIsAvailable(context.DoctorSchedule, args.DoctorId, args.ScheduledDate, args.TimeRange);

        if (context.HasConflict) throw new AppointmentConflictException(DomainErrors.Appointment.Conflict, args.DoctorId, args.ScheduledDate.Add(args.TimeRange.Start));

        return Appointment.Schedule(args.PatientId, args.DoctorId, args.AppointmentTypeId, args.ScheduledDate, args.TimeRange);
    }
