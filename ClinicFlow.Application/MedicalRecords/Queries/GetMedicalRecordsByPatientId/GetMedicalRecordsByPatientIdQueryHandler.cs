...
public class GetMedicalRecordsByPatientIdQueryHandler(IMedicalRecordRepository medicalRecordRepository) :
    IRequestHandler<GetMedicalRecordsByPatientIdQuery, IEnumerable<MedicalRecordDto>>
{
    ...
    {
        var records = await medicalRecordRepository.GetByPatientIdAsync(request.PatientId, cancellationToken);
