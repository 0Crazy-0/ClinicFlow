...
public sealed class CancelAppointmentByStaffCommandHandler(IAppointmentRepository appointmentRepository, IDoctorRepository doctorRepository,
    IMedicalSpecialtyRepository specialtyRepository, IUnitOfWork unitOfWork) : IRequestHandler<CancelAppointmentByStaffCommand>
{
    public async Task Handle(CancelAppointmentByStaffCommand request, CancellationToken cancellationToken)
    {
        ...
        AppointmentCancellationService.CancelByStaff(appointment, new StaffCancellationArgs(request.InitiatorUserId, request.InitiatorRole, specialty, request.Reason));
